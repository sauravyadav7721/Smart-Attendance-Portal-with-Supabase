        ...
        with st.sidebar:
            ...
            if st.button("➕ Add Class"):
                if class_input.strip():
                    exists = supabase.table("classroom_setting").select("*").eq("class_name", class_input).execute().data
                    if exists:
            ...
            if st.button("Delete This Class"):
                if delete_target.strip():
                    ...
                    if st.text_input("Type DELETE to confirm") == "DELETE":
                        ...
                        supabase.table("classroom_setting").delete().eq("class_name", delete_target).execute()
...
def class_controls(supabase):
    try:
        classes_resp = supabase.table("classroom_setting").select("*").execute()
    ...
    with col1:
        if st.button("✅ Open Attendance"):
            if other_open:
                ...
                try:
                    supabase.table("classroom_setting").update({"is_open": True}).eq("class_name", selected_class).execute()
                    st.rerun()
                except Exception:
    ...
    with col2:
        if st.button("❌ Close Attendance"):
            try:
                supabase.table("classroom_setting").update({"is_open": False}).eq("class_name", selected_class).execute()
                st.rerun()
            except Exception:
    ...
    with st.expander("🔄 Update Code & Limit"):
        ...
        if st.button("📏 Save Settings"):
            try:
                supabase.table("classroom_setting").update({"code": new_code, "daily_limit": new_limit}).eq("class_name", selected_class).execute()
